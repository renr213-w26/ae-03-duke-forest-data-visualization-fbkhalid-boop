...
duke_forest |>
  ...
  labs(
    x = "Price ($)",
    y = "Count",
    title = "Histogram of Price of Homes by Garage Availability",
    fill = "Garage Status"
  )
```

The distributions of prices for houses with and without garages are  about the same in shape (both right-skewed). However, the distribution for homes with garages is shifted slightly more to the right, and the most expensive homes aka the outliers tend to have garages. Therefore, having a garage does appear to be associated with a higher price, but it is not the only factor.
## Exercise 2
...
ggplot(duke_forest, aes(x = area, y = price, color = year_built)) +
  ...
  labs(
    ...
    subtitle = "Colored by Year Built",
    color = "Year Built"
  )
```

-   **Claim 1: Larger houses are priced higher.**
True. There is a strong positive relationship between area and price (as area increases, price increases).
-   **Claim 2: Newer houses are priced higher.**
Not strictly true on its own. Looking at the color gradient, lighter points aka the newer homes are found at both low and high prices.    
-   **Claim 3: Bigger and more expensive houses tend to be newer ones than smaller and cheaper ones.**
True. If you look at the top right of the graph (high price, high area), the points tend to be lighter blue (newer). The bottom left (small, cheap) has a mix of dark and light blue.    
::: callout-important
